  ...
  return (
    ...
    >
      ...
      >
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSave({ title, description: desc, link, photos }); }} className="space-y-4">
          <div>
...
export default function Projects() {
  ...
  useEffect(() => {
    ...
    return () => { unsubProjects(); unsubPending(); };
  }, [user?.uid]);
  ...
  const handleAddProject = async (e) => {
    ...
    try {
      await submitPending('project', user.id, user.name, { studentId: user.id, studentName: user.name, title, description, link, photos });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-[#101828] leading-tight">Proyek Pelajar</h1>
      ...
      <AnimatePresence>
        {showAddForm && (
          ...
          >
            <h2 className="text-lg font-bold text-[#101828] mb-4">Tambah Proyek Baru</h2>
      ...
      {isAuthenticated && pending.length > 0 && (
        <div className="mb-8">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {pending.map((p) => (
              <div key={p.id} className="flex-shrink-0 w-56 bg-amber-50 border border-amber-200 rounded-2xl p-4">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between mb-3 gap-2">
                      <span className="inline-flex px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-[#243B3C]/12 text-[#243B3C] border border-[#243B3C]/15 shrink-0">
                        {badge}
                      </span>
                      {canManage && (
                      ...
                      <div className="flex overflow-x-auto gap-2 mb-3 pb-1 -mx-1 px-1 scrollbar-hide">
                        {project.photos.map((ph, idx) => (
                          <img key={idx} src={ph} alt="Dokumentasi"
                            className="w-36 h-24 object-cover rounded-xl shrink-0 border border-[#243B3C]/10" />
                        ))}
                      </div>
                    )}

                    <h3 className="text-[15px] font-bold text-[#101828] mb-1.5 leading-snug">{project.title}</h3>
                    <p className="text-sm text-[#475467] leading-relaxed flex-1 line-clamp-3">{project.description}</p>

                    <div className="mt-4 pt-3 border-t border-[#243B3C]/10 flex items-center justify-between">
                      <span className="text-xs font-semibold text-[#243B3C]">{project.studentName}</span>
                      {project.link && (
                        <a href={project.link} target="_blank" rel="noopener noreferrer"
                          className="p-1.5 rounded-lg text-[#243B3C]/40 hover:text-[#243B3C] hover:bg-[#243B3C]/10 transition-all">
